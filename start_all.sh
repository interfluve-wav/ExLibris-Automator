...
# Python venv before any service (avoids Flask using system python while bot still creates .venv)
export REINSTALL_DEPS=0
bash "$SCRIPT_DIR/scripts/ensure_python_venv.sh"

# Tailwind: templates expect static/css/output.css
if [ ! -f "static/css/output.css" ] && [ -f "package.json" ] && command -v npm >/dev/null 2>&1; then
  ...
  npm run build:css || echo "⚠️  npm run build:css failed; install Node.js or run npm run build:css manually."
fi

if [ ! -f ".env" ]; then
  echo -e "${RED}❌ No .env file. The Discord bot cannot start without credentials.${NC}"
  echo "   Create one: ./start_smart_batch.sh --init-env"
  echo "   Then edit .env (see docs/ENV_EXAMPLE.md) and run ./start_all.sh again."
  echo "   For GUI-only (no Discord), use ./run_standalone.command"
  exit 1
fi
